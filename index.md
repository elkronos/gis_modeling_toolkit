...
`spatialkit` lets the data draw the boundaries.
...
![Raw spatial field and three tessellation methods aggregating it over
North
Carolina](https://raw.githubusercontent.com/elkronos/gis_modeling_toolkit/main/man/figures/readme-tessellations.png)
...
![Random k-fold versus spatial block k-fold assignments across North
Carolina](https://raw.githubusercontent.com/elkronos/gis_modeling_toolkit/main/man/figures/readme-spatial-cv.png)
...
- **Regression only.** A non-numeric response is refused outright, and
  an integer-coded binary response is refused by
  ...
  (A two-valued *non*-integer response, such as a measurement censored
  at a detection limit, is continuous, so it is fitted with a warning.)
  There is no classification path.
- **Vector point data only.** No raster support.
...
| `ranger` | [`fit_rf_model()`](https://elkronos.github.io/gis_modeling_toolkit/reference/fit_rf_model.md), [`cv_rf()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_rf.md) |
...
[`lm()`](https://rdrr.io/r/stats/lm.html) call wired in through
[`new_spatial_fit()`](https://elkronos.github.io/gis_modeling_toolkit/reference/new_spatial_fit.md),
so the whole arc executes without any optional backend.
...
| know how far spatial correlation reaches | [`estimate_sac_range()`](https://elkronos.github.io/gis_modeling_toolkit/reference/estimate_sac_range.md), and [`sac_nugget()`](https://elkronos.github.io/gis_modeling_toolkit/reference/sac_nugget.md) for its nugget |
| build honest CV folds | [`make_folds()`](https://elkronos.github.io/gis_modeling_toolkit/reference/make_folds.md) (`random_kfold`, `block_kfold`, `buffered_loo`, `leave_location_out`, `nndm`) |
| see whether my folds actually separate | [`plot_folds()`](https://elkronos.github.io/gis_modeling_toolkit/reference/plot_folds.md) |
| size the blocks by measurement | [`cv_block_size_sweep()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_block_size_sweep.md), and [`plot()`](https://rdrr.io/r/graphics/plot.default.html) on the result |
| fit a model | [`fit_gwr_model()`](https://elkronos.github.io/gis_modeling_toolkit/reference/fit_gwr_model.md), [`fit_bayesian_spatial_model()`](https://elkronos.github.io/gis_modeling_toolkit/reference/fit_bayesian_spatial_model.md), [`fit_rf_model()`](https://elkronos.github.io/gis_modeling_toolkit/reference/fit_rf_model.md) |
| score it out of sample | [`cv_gwr()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_gwr.md), [`cv_bayes()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_bayes.md), [`cv_rf()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_rf.md) |
| score *my own* learner on the same folds | [`cv_spatial()`](https://elkronos.github.io/gis_modeling_toolkit/reference/cv_spatial.md) + [`new_spatial_fit()`](https://elkronos.github.io/gis_modeling_toolkit/reference/new_spatial_fit.md) |
| score it in sample | [`model_metrics()`](https://elkronos.github.io/gis_modeling_toolkit/reference/model_metrics.md), [`evaluate_insample()`](https://elkronos.github.io/gis_modeling_toolkit/reference/evaluate_insample.md), [`compare_models()`](https://elkronos.github.io/gis_modeling_toolkit/reference/compare_models.md) |
| see the fold-to-fold spread of a CV score | [`plot_cv_metrics()`](https://elkronos.github.io/gis_modeling_toolkit/reference/plot_cv_metrics.md) |
| check whether predicted intervals are the width they claim | [`plot_calibration()`](https://elkronos.github.io/gis_modeling_toolkit/reference/plot_calibration.md) |
| pick predictors without leaking | [`select_features_forward()`](https://elkronos.github.io/gis_modeling_toolkit/reference/select_features_forward.md) ([`gwr_model_selection()`](https://elkronos.github.io/gis_modeling_toolkit/reference/gwr_model_selection.md) for the AICc counterpart) |
| compare backends head to head | [`compare_models_cv()`](https://elkronos.github.io/gis_modeling_toolkit/reference/compare_models_cv.md) |
| check for leftover spatial structure | [`residual_morans_i()`](https://elkronos.github.io/gis_modeling_toolkit/reference/residual_morans_i.md), `plot(fit, type = "variogram")` |
| turn a fit into a map | [`predict_surface()`](https://elkronos.github.io/gis_modeling_toolkit/reference/predict_surface.md) |
| know where that map is extrapolation | [`area_of_applicability()`](https://elkronos.github.io/gis_modeling_toolkit/reference/area_of_applicability.md) |
| free memory held by the caches | [`clear_grid_cache()`](https://elkronos.github.io/gis_modeling_toolkit/reference/clear_grid_cache.md), [`clear_fitted_cache()`](https://elkronos.github.io/gis_modeling_toolkit/reference/clear_fitted_cache.md) |
...
of the same shape is seconds. So it is worth knowing what each backend
is *for* before you spend an afternoon comparing them:
...
exported function has its own page.
...
dissolving your source polygons with
[`st_union()`](https://r-spatial.github.io/sf/reference/geos_combine.html)
is the usual way to get one. Voronoi and `"triangles"` do not need it;
Voronoi falls back to the convex hull of the points.
...
difference, or a column renamed by
[`read.csv()`](https://rdrr.io/r/utils/read.table.html)’s
...
Every backend builds a formula from the column names, and
[`make.names()`](https://rdrr.io/r/base/make.names.html) would silently
rewrite them so the formula and the data disagree. Rename before
...
twice. Use the wrapper’s spelling (`num_trees`, `min_node_size`,
`num_threads`, `mtry`, `importance`, `seed`) and let it pass the value
through.
...
extent and the CRS units to compare against.
[`predict_surface()`](https://elkronos.github.io/gis_modeling_toolkit/reference/predict_surface.md)
refuses above 5,000,000 grid cells for the same reason.
...
object draws the variogram behind it.
...
projected to a CRS the package chose.
...
- [`create_grid_polygons_cached()`](https://elkronos.github.io/gis_modeling_toolkit/reference/create_grid_polygons_cached.md)
  memoises grids keyed on boundary geometry, CRS, target cell count and
  arguments.
  [`clear_grid_cache()`](https://elkronos.github.io/gis_modeling_toolkit/reference/clear_grid_cache.md)
  empties it. It is not a drop-in swap for
  [`create_grid_polygons()`](https://elkronos.github.io/gis_modeling_toolkit/reference/create_grid_polygons.md):
  the cached version also runs
  [`ensure_stable_poly_id()`](https://elkronos.github.io/gis_modeling_toolkit/reference/ensure_stable_poly_id.md),
  so its cells come back re-ordered and re-numbered by a
  projection-independent spatial sort. Use one or the other throughout
  an analysis. Mixing them means two grids over the same boundary whose
  `poly_id` values do not line up.
- [`fitted()`](https://rdrr.io/r/stats/fitted.values.html) on a
  `bayesian_fit` memoises `posterior_epred()` column means in an
  environment carried on the object, because
  ...
  independently. `clear_fitted_cache(fit)` drops it, which is needed
  only if you mutate the engine or the training data by hand after
  fitting.
...
default to `index = 1`, so you have to name index 2 to change what is
printed:
...
`tryCatch(warning = )` will not catch them and
[`suppressWarnings()`](https://rdrr.io/r/base/warning.html) will not
suppress them. Where the documentation says a function *raises* a
warning, it means a genuine R
...
- [`?spatialkit`](https://elkronos.github.io/gis_modeling_toolkit/reference/spatialkit-package.md)
  is the package-level page: it walks the pipeline in order and names
  the function that performs each step.
- Every exported function is documented: see
  [`?fit_gwr_model`](https://elkronos.github.io/gis_modeling_toolkit/reference/fit_gwr_model.md),
...
The package entry cites `spatialkit` itself.
[`citation()`](https://rdrr.io/r/utils/citation.html) also prints the
method references you should cite alongside it if you rely on
[`area_of_applicability()`](https://elkronos.github.io/gis_modeling_toolkit/reference/area_of_applicability.md)
(Meyer & Pebesma 2021), `make_folds(method = "nndm")` (Milà et
al. 2022), the Bayesian GP basis (Riutort-Mayol et al. 2023),
