...
[`?estimate_sac_range`](https://elkronos.github.io/gis_modeling_toolkit/reference/estimate_sac_range.md)
lists the four reasons and what each one means;
...
Four fields say what happened to the data on the way through. On the
clean run above they are all zero, so here is a run where they are not.
