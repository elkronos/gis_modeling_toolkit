...
and twelve regions drawn from where the counties are rather than from
the county boundaries themselves.
...
A region’s `cell_id` is its position in the layer, so it changes if the
layer is re-sorted.
