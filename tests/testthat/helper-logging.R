...
#
# The package logs through logger into the "spatialkit" namespace (see
# R/zzz.R): index 1 appends INFO+ to a temp file, index 2 sends WARN+ to the
# console's error stream (an R message only while knitr is running, which a
# test is not).  Neither raises an R condition, so expect_warning() and
# expect_message() do NOT see these lines -- an assertion written that way
# passes vacuously whether or not the message is emitted.
#
...
capture_spatialkit_log <- function(expr, level = logger::INFO) {
  ...
  # silently disable the console echo.
  #
  # Restores the configuration .onLoad() installs (R/zzz.R: the package's own
  # console appender at WARN on index 2) rather than round-tripping through
  # the getter.
  on.exit({
    logger::log_appender(spatialkit:::.sk_console_appender,
