...
containing a single point, which is a nearest-neighbour interpolation rather
than an aggregation — and it would not be comparable with the ~50-cell grids
below. `get_voronoi_seeds()` clusters the observations first, so cell size
follows sampling density.
...
In-sample fit quality comes from `model_metrics()`; a `spatial_fit` stores the
fitted engine and its settings (`$info`), not precomputed metrics.
