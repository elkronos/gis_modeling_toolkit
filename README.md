...
# random folds cannot see.  (cv_rf() needs the 'ranger' package -- see
# Installation below.)
...
| PSIS-LOO information criterion | `loo`, which `brms` installs and calls for you |
...
```r
# Assigned, because on a machine that HAS the backends this fits both models
# and would otherwise print every fold table and prediction row.
cmp2 <- compare_models_cv(site, "price", "elev", models = c("GWR", "Bayesian"),
                          folds = folds)
...
`predictor_vars` auto-upgrades it to `"combined"` with a note in the session
log file (`tempdir()`; it is INFO-level, so it does not reach the console) —
so the model-aware half is on whenever you hand it a model, whether you asked
or not.
...
#> WARN  estimate_sac_range(): fitted range (25765) exceeds the largest lag
#> the variogram was fitted over (6701 = 1 x cutoff 6701); the empirical
#> variogram never reached a sill, so the range is unidentified rather than
#> long. Returning NA. Raise `cutoff` to fit longer lags, supply
#> `predictor_vars` to detrend, or set a block size explicitly.
...
The checked-in `NAMESPACE` and `man/` are generated by roxygen2 (the version
used is recorded in `DESCRIPTION`); `devtools::document()` reproduces them.
