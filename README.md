...
Every CV function (`cv_gwr()`, `cv_bayes()`, `cv_rf()` and
`cv_spatial()`) accepts a `parallel` argument for fold-level parallelism
via `parallel::mclapply()` (macOS/Linux; falls back to sequential on
Windows with a message). This matters most for `cv_bayes()`, where every
fold is a full MCMC run:
